...
A custom hostname requires Paid access, or a Free workspace whose project shows the ohmyho.st flag (below), and serves the project's Prod deployment. Use the real hostname you want to publish:
...
If plan or apply returns `production_deployment_required` (HTTP 409), Prod has no active deployment yet and nothing was changed. [Deploy to Prod](/environments#deploy-to-prod) or promote the current Dev deployment, then plan again and repeat the apply with the same hostname and key.
...
With the owner's consent, a project can show a small "Powered by ohmyho.st" flag on the right edge of its production site. It loads nothing and links to ohmyho.st. While it shows, a Free workspace may connect its own domain to that project, the custom hostname costs no credits on any plan, and each new Paid period adds 250 credits. Hiding the flag is refused with `powered_by_flag_required` while a Free workspace's domain depends on it.
...
Only an owner can switch it. Agents use MCP `powered_by_flag_get` and `powered_by_flag_set`. After an uncertain answer, repeat the same key and value; every new switch needs a new key.
