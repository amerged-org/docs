...
The required application-auth support set is [Better Auth](https://better-auth.com/docs/installation), customer-owned [WorkOS AuthKit](https://workos.com/docs/authkit/) and [Auth0](https://auth0.com/docs/quickstarts), across Next.js, Vite with or without TanStack Router/Query, and TanStack Start. Public applications need no auth. Keep other existing customer choices; framework/runtime capability checks apply equally to all dependencies. Better Auth has retained real integration proof; completing the same real WorkOS/Auth0 flows is still required before claiming full support. Use the provider's current first-party SDK/guide for the actual browser or server runtime. WorkOS/Auth0 browser integrations use their documented public client identifiers and origin/callback settings; do not request or expose server API/client secrets in a Vite browser bundle. Their server integrations use only customer-owned server runtime secrets.
