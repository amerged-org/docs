...
4. For managed Postgres, use the supported application database binding and versioned migrations. Reach it only through `createPrivateDatabaseClient`; there is no connection string and no acquired connection. Prefer additive schema changes and preserve production records; use the database Skill for sizing or promotion questions.
