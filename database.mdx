...
Use the runtime version `ohmyhost init` reports. For nested JSON results, depth is measured from each row, while the response envelope and all rows still share the same total budgets.
...
The client creates validated plain objects for Workers RPC. An older runtime (0.1.6) created null-prototype objects that RPC rejected; install the runtime alias that `ohmyhost init` reports and update the lockfile to correct this. For an application that stores JSON, verify a JSON write/read through the hosted application, since a scalar-only health query does not exercise that path.
