...
For a library that uses WebAssembly, prefer a runtime-specific package export: a `workerd` entry can statically import the pinned `.wasm` modules, while a separate Node entry loads the same bytes locally. Preserve the library's license and real validation. The platform carries these modules with the deployment artifact; a successful Node build alone does not prove the Worker module can start.
