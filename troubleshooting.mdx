...
| `RUNTIME_CANDIDATE_REJECTED` | The runtime refused the built Worker script. Check the default export and imports of `src/ohmyhost/worker.ts`, then plan the new commit. |
| `STORAGE_JURISDICTION_CONFLICT` | `storage.jurisdiction` differs from the bucket the environment already owns. Keep the existing jurisdiction and plan again. |
